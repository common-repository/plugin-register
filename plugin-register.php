...
function pluginregister_create_table() {
	...
	$check_sql = "SELECT COUNT(1) AS ix_url_exists FROM INFORMATION_SCHEMA.STATISTICS WHERE table_schema=DATABASE() AND table_name='" . $wpdb->prefix . "plugin_register' AND index_name='ix_url'";
	if ($wpdb->get_var($check_sql) == "0"){
		$sql = "create index ix_url on " . $wpdb->prefix . "plugin_register (url)";
		dbDelta( $sql );
	}
	$check_sql = "SELECT COUNT(1) AS ix_url_time_exists FROM INFORMATION_SCHEMA.STATISTICS WHERE table_schema=DATABASE() AND table_name='" . $wpdb->prefix . "plugin_register' AND index_name='ix_url_time'";
	if ($wpdb->get_var($check_sql) == "0"){
		$sql = "create index ix_url_time on " . $wpdb->prefix . "plugin_register (url, time)";
		dbDelta( $sql );
	}
}
...
$register->Register(); // leave this as it is</textarea>
